...
# Wrapper script to launch UR5 with RViz without snap library conflicts
...
cat > /tmp/launch_ur5_clean.sh << 'EOFSCRIPT'
#!/bin/bash
...
echo "=== UR5 Launch (Clean Environment) ==="
echo "Robot IP: $ROBOT_IP"
echo "Use Fake Hardware: $USE_FAKE_HARDWARE"
echo "========================================="

# Launch the UR5 controller
...
chmod +x /tmp/launch_ur5_clean.sh

# Execute in clean environment
exec /tmp/launch_ur5_clean.sh "$SAVED_HOME" "$SAVED_USER" "$SAVED_SHELL" "$SAVED_TERM" "$SAVED_DISPLAY" "$ROBOT_IP" "$USE_FAKE_HARDWARE"
