...
public:
    MoveGroupReceiver() : Node("move_group_reciever") {
        // FALLBACK: Initialize with safe default pose
        ...
        position_subscriber_ = this->create_subscription<geometry_msgs::msg::Pose>(
            ...
            [this](geometry_msgs::msg::Pose::SharedPtr msg) {
                ...
                // FALLBACK: Mark that we've received valid pose data
    ...
    // FALLBACK: Add method to check if pose data has been received
    bool hasReceivedPose() const { return has_received_pose_; }
...
private:
    ...
    // FALLBACK: Track if we've received valid pose data
...
int main(int argc, char** argv)
{
    ...
    while(true){
        ...
        // FALLBACK: Use default fruit width if no detection data received
        float fruit_width = move_group_reciever->getFruitWidth();
        if (fruit_width <= 0.0) {
            fruit_width = 0.040; // Default safe gripper width
            RCLCPP_WARN(LOGGER, "No fruit width data received, using default: %f", fruit_width);
        }
        
        geometry_msgs::msg::Pose target_pose1 = move_group_reciever->getTargetPose();
        
        // FALLBACK: Log if using default pose vs received pose
        if (!move_group_reciever->hasReceivedPose()) {
            RCLCPP_WARN(LOGGER, "No target pose received from detection, using fallback position");
        }
